...
- 2026-06-11 (perf, „pokračuj bez api"): Vendor code-splitting v `vite.config.ts` (`manualChunks`: react-vendor + genai-vendor). Největší chunk klesl 653 kB → 333 kB (varování „chunks larger than 500 kB" zmizelo); genai SDK (308 kB / 59 kB gzip) izolovaný a cacheovatelný napříč deploymenty. Total beze změny — cíl byl cache + paralelní stažení + odstranění oversized chunku, ne zmenšení kódu. Bez změny app logiky, `tsc` 0 chyb, build OK (commit 50d0c0c). **Hlubší perf (lazy-load genai + modalů přes React.lazy) NEuděláno záměrně** — sahá na App.tsx, který je v tomto ISA explicitně Out of Scope, a nese vyšší riziko; čeká na samostatné rozhodnutí. Zbývá pouze security (API klíč v bundlu, odloženo).
- 2026-06-11 (verify + push): Ověření servírovaného produkčního buildu (vite preview :4178, curl). **Browser-tool gap přiznán:** ani Interceptor ani agent-browser na tomto WSL hostu nejsou → ověření je artefaktové (HTTP 200 + valid JS + shell), NE vizuální render. Výsledky: `index.html` 200 + `<div id="root">` + správný title; 3 JS chunky (index/genai-vendor/react-vendor) 200, `text/javascript`, `node --check` parse OK; `manifest.json` 200 `application/json`; v bundlu zapečené **v1.8.1**; ladicí `console.log "Gemini Live Connected"` v produkčním artefaktu **0×** (úklid potvrzen end-to-end). Historie úprav doplněna do `RELEASE_HISTORY` v1.8.1 (poznámka o rychlejším načítání). **Pushnuto na origin/main** (`1134448..d1ab426`, 12 commitů), větev synchronizovaná.
